...
import GameRequest, { IGameRequestModel } from './game-request.models';
...
const createGameRequest = async (req: AuthRequest, res: Response, next: NextFunction) => {
  ...
  return gameRequest.save()
    .then(gameRequest => res.status(201).json(responseGameRequest))
...
const readGameRequest = async (req: AuthRequest, res: Response, next: NextFunction) => {
  ...
  if (isCreator === 'true') {
    try {
      const gameRequest: IGameRequestModel | null = await GameRequest.findById(gameRequestId)
...
const readAll = async (req: AuthRequest, res: Response, next: NextFunction) => {
  ...
  try {
    let gameRequests: IGameRequestModel[] = await sortGameRequests(+sort, filters, true)
      ...
      .select('-booked -__v'); // get rid of field
    let total = await sortGameRequests(+sort, filters, false).count(); //make true for future games only
...
export default { createGameRequest, readGameRequest, readAll, updateGameRequest };
